...
function getClientCredentialsTokenByTenant(tenant,credentials,zoneInfo, cb) {
  let credUrl = credentials.url || credentials.uaa.url;
  const credUaaDomain = credentials.uaadomain || credentials.uaa.uaadomain;
  const identityZone = credentials.identityzone || credentials.uaa.identityzone;
  const zoneId = zoneInfo && zoneInfo.zoneId;
  const targetTenantUrl = !zoneId ? credUrl.replace(identityZone, tenant) : 'https://' + credUaaDomain;
