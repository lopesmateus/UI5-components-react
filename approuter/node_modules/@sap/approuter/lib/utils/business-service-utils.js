...
module.exports = {
  ...
  getDestinationCredentialsBySapCloudService(req, sapCloudService, cb){
    ...
    module.exports.fetchDestinationBSCredentials(req, tracer, function (err, destinationBSCredentials) {
      ...
      if (destinationBSCredentials[sapCloudService].credentials.destinationId){
        for (let sapCloudServiceKey in destinationBSCredentials){
          if (destinationBSCredentials[sapCloudServiceKey].credentials.destinationId === destinationBSCredentials[sapCloudService].credentials.destinationId
              ...
              && sapCloudServiceKey !== sapCloudService
      ...
      cb(null, destinationBSCredentials[sapCloudService], relatedBusinessServicesXsappNames);
    });
  },
      ...
      if (destination.hasOwnProperty('sap.cloud.service')) {
        if (destination.hasOwnProperty('app_host_id')) {
          ...
          if (html5RepoDestinations[sapCloudService]){
            ...
            html5RepoDestinations[sapCloudService]['xsappname'] =
                html5RepoDestinations[sapCloudService]['xsappname'] + ',' + destination.xsappname;
          } else {
          ...
          if (destination.destinationId){
            let destinationKey = tenant + '_' + destination.destinationId;
            if (req.app.services[destinationKey]) {
...
function addAppHostIdData(destinationBSCredentials, html5RepoDestinations){
  ...
  for (let sapCloudService in html5RepoDestinations) {
    if (destinationBSCredentials[sapCloudService] && destinationBSCredentials[sapCloudService].credentials) {
      destinationBSCredentials[sapCloudService].credentials['html5-apps-repo'] = {
        app_host_id: html5RepoDestinations[sapCloudService].appHostId,
        xsappname: html5RepoDestinations[sapCloudService].xsappname
      };
    }
  }
}
...
function getDestinations(req,cb){
  ...
  if (req.destinationKey) {
    ...
    destinationUtils.getServiceInstanceDestinationsByTenant(req, function(err, instanceDestinations){
      ...
      destinationUtils.getDestinationsByTenant(req.tenant, null,req.zoneInfo,function (err, subaccountDestinations) {
        if (err) {
          return cb(err);
        }
